...
const ServiceDetailsBanner = () => {
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.border = `${borderWidth} solid ${Math.random() > 0.5 ? '#C7AC5F40' : '#DCDFE040'}`;
  ...
  return (
    <div ref={bannerRef} className="min-h-[50vh] sm:min-h-[60vh] md:min-h-[70vh] w-full page-banner relative overflow-hidden">
      ...
      <div className="absolute top-3 left-3 sm:top-5 md:top-10 sm:left-5 md:left-10 w-16 sm:w-20 md:w-32 h-16 sm:h-20 md:h-32 border border-[#C7AC5F] rounded-full opacity-10 ai-pulse"></div>
        ...
        >
          ...
          <div className="w-full flex flex-col items-center mb-4 sm:mb-6">
            ...
            <div className="relative mb-4 sm:mb-5">
              <div className="absolute w-20 h-20 rounded-full bg-[#C7AC5F30] filter blur-lg ai-pulse"></div>
              <img 
                src={service?.icon} 
                alt={title} 
            ...
            >
              {title}
              <span className="absolute -bottom-3 left-0 right-0 mx-auto w-[100px] sm:w-[150px] h-[3px] bg-[#C7AC5F] ai-shimmer"></span>
            </h1>
          </div>
          ...
          <div
            data-aos="fade-up"
            className="text-sm sm:text-base font-medium px-4 sm:px-5 py-2 sm:py-3 w-fit flex flex-wrap justify-center text-white items-center gap-2 sm:gap-3 bg-[#00000080] backdrop-blur-sm rounded-full border border-[#C7AC5F30] shadow-lg"
          >
            <Link to="/" className="hover:text-[#C7AC5F] transition-colors duration-300">Home</Link>
            <FiArrowRight className="text-[#C7AC5F] text-xs sm:text-sm" />
            <Link to="/services" className="hover:text-[#C7AC5F] transition-colors duration-300">Services</Link>
            <FiArrowRight className="text-[#C7AC5F] text-xs sm:text-sm" />
            <span className="text-[#C7AC5F] font-bold truncate max-w-[120px] sm:max-w-none">{title}</span>
          </div>
        </div>
      </div>
