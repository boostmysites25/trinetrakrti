...
const AIFloatingElements = ({ count = 10, className = "" }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      element.style.border = `1px solid ${isGold ? '#C7AC5F20' : '#DCDFE020'}`;
      ...
      if (Math.random() > 0.8) {
        ...
        element.style.backgroundImage = `
          radial-gradient(circle at center, ${isGold ? '#C7AC5F10' : '#DCDFE010'} 0%, transparent 70%),
          linear-gradient(to right, transparent 49%, ${isGold ? '#C7AC5F20' : '#DCDFE020'} 50%, transparent 51%)
        `;
    ...
    for (let i = 0; i < Math.min(count / 5, 3); i++) {
      ...
      hexagon.style.border = `1px solid ${isGold ? '#C7AC5F30' : '#DCDFE030'}`;
